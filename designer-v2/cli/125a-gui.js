...
function validate(p) {
  ...
  const shapes = ['studio','flat','tapered','cylindrical','stepped'];
  const materials = ['plastic-black','metal-dark','aluminium','steel','brass','rubber'];
  const lights = ['soft','neutral','contrast','dramatic'];
  const indicators = ['line','dot','notch','groove'];
...
const presets = {
  'studio-dark': {
    'design.shape':'studio',
    'design.material':'metal-dark',
    'design.baseColor':'#242A31FF',
    'design.accentColor':'#657A8FFF',
    'lighting.preset':'neutral'
  },
  'industrial-steel': {
    'design.shape':'stepped',
    'design.material':'steel',
    'design.baseColor':'#59616AFF',
    'design.accentColor':'#2C3137FF',
    'lighting.preset':'contrast'
  },
  'vintage-brass': {
    'design.shape':'tapered',
    'design.material':'brass',
    'design.baseColor':'#806A3CFF',
    'design.accentColor':'#C3A35FFF',
...
if (cmd === 'preset') {
  ...
  if (!presets[name]) fail(`Unknown preset: ${name}`);
  const p = readJson(file);
  for (const [k,v] of Object.entries(presets[name])) setPath(p,k,v);
  const errors = validate(p);
